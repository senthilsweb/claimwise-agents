...
| `BEDROCK_MODEL_ID` | yes | The exact model or inference-profile ID to call. Never hardcoded anywhere in the code — some models need the inference-profile ID instead of the bare model ID (see [Operations](operations.md#runbook)). |
...
| `DATABRICKS_HOST` / `DATABRICKS_HTTP_PATH` / `DATABRICKS_TOKEN` | if `AGENT_TARGET=databricks` | Read-only warehouse credentials. Agents have no code path that can write — see [Architecture](architecture.md#tool-calling). |
...
[Operations](operations.md#tracing) for what each one shows you.
...
- See these variables in action → [Examples](examples.md)
- What each agent does with these settings → [Architecture](architecture.md)
